...
const PostScreen = props => {
  ...
  const data = [
    {
      ...
      content:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla nec diam justo. Nullam sed volutpat nunc.',
    },
    {
      id: 2,
      title: 'Bai dang 2',
  ...
  const renderItem = (item, index) => {
    return (
      <ItemContainer
        style={{borderBottomWidth: index === data.length - 1 ? 0 : 4}}>
        <ImagePost source={item.image} />
        <Gap $height={10} />
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('Main', {screen: 'PostDetail', params: {item}})
          }>
          <Text15Medium $color={colors.dark}>{item.title}</Text15Medium>
          <Gap $height={5} />
          <Text14 numberOfLines={2} $color={colors.dark}>
            {item.content}
          </Text14>
        </TouchableOpacity>
      </ItemContainer>
