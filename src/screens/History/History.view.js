...
import { Container, Gap, HStack, HStackBetween, HStackEnd, HStackStart } from "../../components/Layout";
...
const HistoryScreen = ({ navigation }) => {
    ...
    return (
        <Container>
            <HStack>
                <TextBold $fontSize={'24px'} $color={colors.dark}>Lịch sử</TextBold>
            </HStack>
            <Gap $height={30} />
            <View>
                {dataHistory.map((item) => (
                    <HistoryList
                        ...
                        <HStackEnd>
                            <HistoryStatus $bgColor={item.status === 1 ? colors.green1 : colors.red1}>
                                <Text13 $color={colors.gray3}>{statusHistory(item.status)}</Text13>
                            </HistoryStatus>
                        </HStackEnd>
                    </HistoryList>
                ))}
