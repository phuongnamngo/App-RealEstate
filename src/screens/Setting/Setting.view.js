...
const SettingScreen = props => {
  ...
  const {infoUser, logoutSubmit, refreshing, onRefresh} = useSetting();
    ...
    <Container>
      <ScrollView
        ...
        <VStack>
          ...
          <Gap $height={10} />
          <TextBold $fontSize={'20px'} $color={colors.dark}>
            {infoUser.firstname
              ? infoUser.firstname + ' ' + infoUser.lastname
              : 'Username'}
          </TextBold>
          <Gap $height={10} />
          <TextTotal $fontSize={'20px'} $color={colors.dark}>
            Tổng điểm bạn đang có: 200 điểm
          </TextTotal>
        </VStack>
        <Gap $height={40} />
        {data.map((item, index) => {
          return (
            <SettingItem
              onPress={() =>
                navigation.navigate('Main', {screen: item.onPress})
              }
              key={index}
